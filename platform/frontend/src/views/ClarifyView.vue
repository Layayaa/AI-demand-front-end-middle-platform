    ...
    <div v-else-if="clarification" class="clarify-layout">
      <section class="card clarify-chat">
        ...
        <div v-if="canReply" class="chat-composer">
          ...
          <p class="chat-attachment-hint">
            {{ uploadingAttachments ? '附件上传中…' : '没有材料也不影响使用；如有相关表格或文档，可附加后减少重复说明。' }}
          </p>
        </div>
      ...
      <aside class="clarify-side">
        ...
        <section class="card card-pad gap-card">
          <div class="section-label">Open questions / 还需要了解</div>
          <div class="card-title mt8">影响方案的核心问题</div>
          <div v-if="coreGaps.length" class="gap-list">
            <div v-for="gap in coreGaps" :key="gap.key" class="gap-item">
              <strong>{{ gap.label }}</strong>
              <span>{{ gap.reason }}</span>
            </div>
          </div>
          <p v-else class="card-sub">影响方案的核心信息已经基本齐全。</p>
          <details v-if="supplementaryGaps.length" class="supplementary-gaps mt16">
            <summary>可在评审阶段补充（{{ supplementaryGaps.length }}项）</summary>
            <div class="gap-list mt8">
              <div v-for="gap in supplementaryGaps" :key="gap.key" class="gap-item muted-gap">
                <strong>{{ gap.label }}</strong>
                <span>{{ gap.reason }}</span>
              </div>
            </div>
