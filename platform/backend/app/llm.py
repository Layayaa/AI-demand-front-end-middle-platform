...
- 同一句或同义问题不能连续出现。用户已经连续确认同一口径时，应把它写入 facts 并转向下一个真正缺口，不能退回泛化的“请继续补充”。
- 用户说“如果影响方案请继续指出”“继续问”时，视为明确要求继续澄清，必须选择一个尚未回答的高影响缺口，不能立即 ready_to_confirm。
- 如果画像中存在 materialCandidates，先让业务人员在材料确认台处理；不得绕过候选状态把材料内容直接写入 facts，也不要再次要求用户复述附件中的步骤。
- 首轮问题必须紧扣需求标题和摘要，优先询问一个能区分目标、当前做法或最终结果的具体问题；禁止使用“请继续补充关键事实”这类泛化开场。
- 从第 6 轮开始主动收敛：如果剩余信息只影响人数、耗时、频率或价值估算，应 ready_to_confirm，并把这些留到评审阶段补充。
- 第 10 轮是默认上限：除非需求类型、业务对象、当前做法或人工边界仍有一个完全未知，否则必须 ready_to_confirm，不能继续盘问。
...
需求摘要：{project.get("summary") or "无"}
所属部门：{project.get("department") or "无"}
当前已完成业务回答轮数：{user_rounds}（建议 6 轮后收敛，默认最多 10 轮）
