...
const { stripDeep } = require('./strip-emoji');
...
function load() {
  ...
  try {
    ...
    if (fs.existsSync(DB_FILE)) {
      ...
      const before = JSON.stringify(db.requirements);
      db.requirements = stripDeep(db.requirements);
      if (JSON.stringify(db.requirements) !== before) save();
      return db;
    }
  } catch (e) {
